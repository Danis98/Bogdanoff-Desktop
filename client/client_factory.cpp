...
void create_clients(const json& config_json, std::vector<std::unique_ptr<Client>>& clients, const std::vector<std::unique_ptr<Price>>& prices)
{
    for(const auto& client_config : config_json)
    {
        ...
        size_t price_idx;
        for(price_idx = 0; price_idx < prices.size(); price_idx++)
        {
            if(prices[price_idx]->get_name() == client_config["price"])
                break;
        }
        if(price_idx == prices.size())
        {
        ...
        if(client_config["venue"] == "coinex")
            clients.push_back(std::make_unique<CoinexClient>(client_config, *prices[price_idx]));
        else if(client_config["venue"] == "etherscan")
            clients.push_back(std::make_unique<EtherscanClient>(client_config, *prices[price_idx].get()));
        else if(client_config["venue"] == "blockchain_info")
            clients.push_back(std::make_unique<BlockchainInfoClient>(client_config, *prices[price_idx].get()));
        else continue;
    }
}